  ...
  useEffect(() => {
    const checkAndApplyPalette = async () => {
      ...
      if (user && settings.colorPalette && settings.colorPalette !== window.__GLOBAL_PALETTE__) {
        // Only apply if user has a custom palette (not default)
        if (settings.colorPalette !== 'copper-brown' && settings.colorPalette !== 'default') {
          applyColorPalette(settings.colorPalette);
        }
        // If user palette is default/copper-brown, do not override global palette
      }
