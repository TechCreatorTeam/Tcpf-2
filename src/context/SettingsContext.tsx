...
export const SettingsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const checkAndApplyPalette = async () => {
      ...
      if (user) {
        // For authenticated users, apply their custom palette if it's different from global
        if (settings.colorPalette && settings.colorPalette !== window.__GLOBAL_PALETTE__) {
          applyColorPalette(settings.colorPalette);
        }
      } else {
        // For non-authenticated users, only apply if no global palette is set
        if (!window.__GLOBAL_PALETTE__ && settings.colorPalette) {
          applyColorPalette(settings.colorPalette);
        }
      }
    };
  ...
  const loadSettings = async () => {
    try {
      ...
      // Check if we have a global palette that should take precedence
      const globalPalette = window.__GLOBAL_PALETTE__;
      ...
      if (localSettings) {
        ...
        // If there's a global palette and user doesn't have a custom one, use global
        if (globalPalette && (!parsed.colorPalette || parsed.colorPalette === 'default' || parsed.colorPalette === 'copper-brown')) {
          mergedSettings.colorPalette = globalPalette;
        }
      ...
      if (!localSettings) {
        ...
        // Use global palette if available
        if (globalPalette) {
          finalSettings.colorPalette = globalPalette;
        }
