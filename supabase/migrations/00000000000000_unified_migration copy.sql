...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'inquiries' AND column_name = 'client_name') THEN
    ALTER TABLE inquiries ADD COLUMN client_name text;
  END IF;
...
-- Ensure tables exist before any references
